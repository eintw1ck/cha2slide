...
static void footer()
{
    ...
    char *bob[13] = {
        ...
        "Let's get crazy.",
        "The secret to doing anything is believing that you can do it. Anything that you believe you can do strong "
        ...
        "Exercising the imagination, experimenting with talents, being creative; these things, to me, are truly the "
        "windows to your soul.",
        "Mix up a little more shadow color here, then we can put us a little shadow right in there. See how you can "
        "move things around? You have unlimited power on this canvas -- can literally, literally move mountains",
...
static void usage()
{
    fprintf(stderr, "Usage: cha2slide <command> <input-file> <output-file> <command-dependent-argument>\n"
                    "Commands:\n"
                    "\tencrypt\n"
                    "\tdecrypt\n"
...
int encode(FILE *input, FILE *output, uint32_t width)
{
    ...
    if ((uint32_t)size % width)
        pprintf(LOG_WARNING,
                "Input data length (%d) is not divisable by given width (%d). Truncating data, this may output an "
                "incorrect image.",
                size, width);
    else
